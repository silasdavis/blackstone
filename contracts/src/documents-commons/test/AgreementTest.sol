...
contract AgreementTest {
     ...
     */
	function testModifiers() external returns (string) {

        TestAgreement modAgreement;

        // onlyBySignatory
		modAgreement = new TestAgreement("modAgreement1");
		if (modAgreement.modifyByOnlyBySignatory(this)) return "onlyBySignatory for owner should fail";
		if (modAgreement.modifyByOnlyBySignatory(msg.sender)) return "onlyBySignatory with msg.sender should fail";
		modAgreement.addSignatory(this);
		if (!modAgreement.modifyByOnlyBySignatory(this)) return "onlyBySignatory should pass after adding owner";
		...
		if (!modAgreement.modifyByOnlyByOwnerOrSignatory(this)) return "onlyByOwnerOrSignatory should pass for owner";
	 ...
	 */
	function testSignatoryManagement() external pre_cleanSignatories returns (string) {
     ...
     */
    function testVersionSigning() external pre_cleanSignatories returns (string) {
		...
		signatories.push(party2);
		signatories.push(party3);
		signatories.push(this);
        agreement.addSignatories(signatories);

        // Sign first version with 2/4 signatories and check states
		agreement.addVersion(versionHash1);
        if (party1.signAgreement(agreement, versionHash1) != BaseErrors.NO_ERROR()) return "Party1 proxy encountered an error when signing version 1";
        if (party2.signAgreement(agreement, versionHash1) != BaseErrors.NO_ERROR()) return "Party2 proxy encountered an error when signing version 1";
        ...
        if (party2.signAgreement(agreement, versionHash2) != BaseErrors.NO_ERROR()) return "Party2 proxy encountered an error when signing version 2";
        ...
        if (party3.signAgreement(agreement, versionHash2) != BaseErrors.NO_ERROR()) return "Party3 proxy encountered an error when signing version 2";
        if (agreement.isFullyConfirmed(versionHash2)) return "Version 2 should not be fully confirmed, yet.";
        if (party1.signAgreement(agreement, versionHash2) != BaseErrors.NO_ERROR()) return "Party1 proxy encountered an error when signing version 2";
...
contract TestAgreement is Agreement {
    ...
    constructor(string _name) Agreement(_name) public {}
